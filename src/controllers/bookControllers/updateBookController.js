...
const updateBookController = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const { bookNumber, bookName, summary, author, genre, image, bgColor, totalBooks, isOnline, onlineFileUrl, almirahNumber, shelfNumber } = req.body;
    try {
        // Check if book exists
        const existingBook = yield prismaDb_1.prisma.book.findFirst({
            where: {
                id: bookId,
        ...
        if (newTotal > currentTotal) {
            ...
            const unborrowedCopies = yield prismaDb_1.prisma.bookCopy.findMany({
                where: {
                    bookId: bookId,
                    isBorrowed: false
                },
            ...
            if (unborrowedCopies.length > 0) {
                yield prismaDb_1.prisma.bookCopy.deleteMany({
                    where: {
        ...
        const updatedBook = yield prismaDb_1.prisma.book.update({
            where: { id: bookId },
            data: updateData
        });
        ...
        console.error('Update book error:', (error === null || error === void 0 ? void 0 : error.message) || error);
